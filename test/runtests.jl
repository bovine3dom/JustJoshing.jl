...
@testset "just_joshing.jl" begin
    ...
    @test all(derivative.(x->C(1,0.5;K=x),rand(100_000)*10) .<= 0)
    ...
    @test all(derivative.(x->C(1,0.5;σ=x),rand(1000)) .>= 0)

    # V: if d=0 (always true in our model) then price should increase as we get closer to expiry
    @test all(derivative.(T->C(1,0;T=T),rand(1000)) .>= 0)
    ...
    @test all(derivative.(x->derivative.(K->C(1,0;K=K),x),rand(1000)) .>= 0)
