...
Plots.plot(x->C(0.5,0;σ=x),0:0.01:0.35) # Call options with volatile underlyings are more expensive (Fig 3.8, page 66)


# Price of call option should monotonically increase with volatility
@assert all(derivative.(x->C(1,0.5;σ=x),rand(100_000).*0.99) .>= 0)
